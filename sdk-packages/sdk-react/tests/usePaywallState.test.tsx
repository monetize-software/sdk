...
describe('usePaywallState', () => {
  it('возвращает initial snapshot и реагирует на onStateChange', () => {
    const fake = new FakePaywall({
      initialState: { open: false, view: null, error: null }
    });
    ...
    expect(result.current).toEqual({ open: false, view: null, error: null });

    act(() => {
      fake.setState({ open: true, view: 'layout', error: null });
    });

    expect(result.current).toEqual({ open: true, view: 'layout', error: null });
  });
});
