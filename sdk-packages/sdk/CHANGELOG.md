...
- b485bcd: Edge-mirror failover — the SDK survives state-level IP blocking of the primary custom domain
  ...
  - **Analytics follow the failover.** The events endpoint resolves through `billing.activeApiOrigin()` at flush time, so trackers keep delivering after a switch. `ApiGatewayClient` never replays streams, fails over only on hard connect-level errors (a hedge-expired AI call may already be processing — and charging — server-side), and its caller aborts now surface as `aborted` (previously `network_error`).
...
- 958afbd: Email-confirm signup flow resumes automatically
...
- 51cef86: Analytics: `purchase_completed{restored:true}` is no longer tracked; `paywall_closed` only pairs with a tracked `paywall_viewed`
...
- c86824c: Blind `paywall.open()` for a user with an active subscription no longer shows any modal
...
- 5cf261c: AuthPanel surfaces the `custom_domain_required` auth error with an actionable message ("the paywall has no custom domain configured") instead of the generic fallback — merchant misconfiguration is visible right in the UI during setup. OAuth redirects and confirmation/recovery email links are built from the paywall's custom domain, so the backend hard-fails auth without it.

## 3.2.2

### Patch Changes

- d6af664: Extension: support-ticket attachments actually reach the ticket; attachment limit is now 5MB per file
...
- b015275: Extension analytics: `paywall_viewed` / `paywall_closed` are gated to the real paywall view (`'layout'`), matching the base SDK
...
- ebc44ef: Recover from server-side revoked sessions instead of dead-ending with an opaque `http_401` "Request failed"
...
- 5d98a33: Per-open custom paywall title: `paywall.open({ title: 'Unlock export' })`
...
- 5d98a33: A/B price experiments (SDK side)
...
- a30e45b: Fix unstyled paywall in sdk-extension (Tailwind not compiled in the extension build)
...
- 71101d6: Fix paywall overlay not showing when an ancestor of the host establishes a containing block
