...
describe('AuthClient', () => {
  ...
  it('signUp returns already_registered without setting session', async () => {
    // GoTrue anti-enumeration: an existing email (incl. OAuth-only) comes back as
    // already_registered from the backend, not confirmation_required — the UI must
    // send the user to sign in, not to a fake "check your email" screen.
    const fetchMock = vi.fn<typeof fetch>(async () =>
      jsonResponse({ status: 'already_registered', email: 'taken@b.c' })
    );
    const auth = new AuthClient({
      paywallId: PAYWALL_ID,
      apiOrigin: API_ORIGIN,
    ...
    const res = await auth.signUp({ email: 'taken@b.c', password: 'pw' });

    expect(res).toEqual({ kind: 'already_registered', email: 'taken@b.c' });
    expect(auth.getCachedSession()).toBeNull();
  });
